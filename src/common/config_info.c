...
get_configdata(const char *my_exec_path, size_t *configdata_len)
{
    ...
    *configdata_len = 23;
    ...
    configdata[i].setting = pstrdup("<DOCDIR is unavailable in the libpq standalone build>");
    i++;

    configdata[i].name = pstrdup("HTMLDIR");
    configdata[i].setting = pstrdup("<HTMLDIR is unavailable in the libpq standalone build>");
    i++;
    ...
    configdata[i].setting = pstrdup("<INCLUDEDIR-SERVER is unavailable in the libpq standalone build>");
    i++;
    ...
    configdata[i].setting = pstrdup("<MANDIR is unavailable in the libpq standalone build>");
    i++;
    ...
    configdata[i].setting = pstrdup("<PGXS is unavailable in the libpq standalone build>");
    i++;
